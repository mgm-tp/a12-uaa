...
import * as UaaAction from "../actions.js";
import { reduxStore } from "../utils/index.js";

import { NotAuthorizedError } from "./Errors.js";

/**
 * Filter which is in charge of handling 401 error.
 */
export class ResponseFilter401 implements ResponseFilter {
	...
	doResponseFilter(response: Response | undefined): ResponseFilterResult {
		if (response && !response.ok && response.status === 401) {
			if (reduxStore.getStore()) {
				reduxStore.dispatch(UaaAction.unauthorized({ url: response.url }));
			}
			return {
				response: response,
				continue: true,
