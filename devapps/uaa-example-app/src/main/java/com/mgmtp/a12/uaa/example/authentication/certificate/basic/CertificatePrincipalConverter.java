 ...
 */
package com.mgmtp.a12.uaa.example.authentication.certificate.basic;
...
public class CertificatePrincipalConverter implements CertificateConverter {
	...
	@Inject
	private UAAExtendedPrincipalDataLoader extendedUserDataLoader;

	@Override
	public UserDetails convert(@Nonnull X509Certificate certificate) {
		String userNameValue = getAttributeValueFromCertificatePrincipal(certificate, COMMON_NAME);
		String roles = getAttributeValueFromCertificateExtension(certificate, USER_ROLE);
	...
	private String getAttributeValueFromCertificatePrincipal(X509Certificate certificate, String attributeName) {
		try {
			...
			LOGGER.error("Has error when get attribute from certificate", e);
		}
		return null;
	}

	public String getAttributeValueFromCertificateExtension(X509Certificate certificate, String attributeName) {
		try {
			...
			LOGGER.error("Has error when get attribute value from certificate extension", e);
		}
		return null;
	}
}
