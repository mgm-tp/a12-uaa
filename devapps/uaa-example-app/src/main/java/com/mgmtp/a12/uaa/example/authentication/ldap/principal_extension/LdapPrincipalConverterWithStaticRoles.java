 ...
 */
package com.mgmtp.a12.uaa.example.authentication.ldap.principal_extension;
...
@Profile("principal")
public class LdapPrincipalConverterWithStaticRoles extends LdapUserDetailsMapper {
	...
	public UserDetails mapUserFromContext(DirContextOperations ctx, String userName, Collection<? extends GrantedAuthority> authorities) {
		authorities = createAuthorities(userName);
	...
	private Collection<? extends GrantedAuthority> createAuthorities(String username) {
		return (("admin".equals(username))
			? Arrays.asList(new Role.Builder("Admin").build(), new Role.Builder("Manager").build(), new Role.Builder("Reviewer").build())
			: Arrays.asList(new Role.Builder("Reviewer").build()));
