...
public class IdentityPropertyValidator extends BaseKeywordValidator {
	...
	public void validate(ExecutionContext executionContext, JsonNode node, JsonNode rootNode, NodePath instanceLocation) {
		...
		if (identityField.isString() && node.isObject()) {
			...
			String field = identityField.asString();
			...
			if (!localIdentityValues.add(identity)) {
				executionContext.addError(
					error()
						.message("{0}: Duplicate identity property [{1}] with value [{2}]")
						.arguments(instanceLocation, field, identity.toString())
						.instanceLocation(instanceLocation)
						.instanceNode(node)
						.evaluationPath(executionContext.getEvaluationPath())
						.build()
			...
			identityMap.entrySet().stream()
				.filter(entry -> !entry.getKey().equals(currentFileName) && entry.getValue().contains(identity))
