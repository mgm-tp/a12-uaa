...
public class UAARestClientPropertiesBuilderTest {
	...
	void setUp() {
		...
		OidcConfiguration.PublicClientConfiguration publicClientProperties = new OidcConfiguration.PublicClientConfiguration();
		publicClientProperties.setClientId("clientIdTest");
		publicClientProperties.setRealmName("realmNameTest");
		publicClientProperties.setIdpBaseUrl("http://localhost:9090");
		publicClientProperties.setTokenRenewThresholdInSeconds(60);

		publicClientProperties.setSsoConfiguration(ssoConfiguration);
		publicClientProperties.setLoginRedirectRelativeUrl("/auth");
		publicClientProperties.setTokenExchangeRelativeUrl("/token");
		publicClientProperties.setLoginRedirectRelativeUrl("/callback");
		oidcConfiguration.setPublicClient(publicClientProperties);
	...
	@Test
	void buildTest() {
		UAARestClientProperties uaaRestClientProperties = UAARestClientPropertiesBuilder.with(clientSelfconfiguration)
			.withBaseUrl(BASE_URL)
			.withUsername("admin")
			.withPassword("admin")
			.withAuthenticationType(AuthenticationType.OAUTH2.name())
			.withExcludedContexts(clientSelfconfiguration.getExcludedDelegatedContexts())
			.build();

		Assertions.assertEquals(BASE_URL, uaaRestClientProperties.getUaaBase().getUrl());
		Assertions.assertEquals("admin", uaaRestClientProperties.getAuthenticationConfiguration().getUsername());
		Assertions.assertEquals("admin", uaaRestClientProperties.getAuthenticationConfiguration().getPassword());
		Assertions.assertEquals(clientSelfconfiguration.getOidc().getPublicClient().getSsoConfiguration().getPasswordXpath(),
