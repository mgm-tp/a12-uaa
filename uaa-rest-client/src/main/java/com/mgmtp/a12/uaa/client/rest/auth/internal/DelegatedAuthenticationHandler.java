...
/**
 * This is implementation is used when the authentication is delegated to other service.
 * You might use {@link DelegatingAuthenticationFilter} to propagate credential context
 *
 */
public class DelegatedAuthenticationHandler implements AuthenticationHandler {

	@Override
	public AuthorizationData authenticate() {
		AuthorizationDataStore authDataHolder = AuthorizationDataHolder.getCredentialContext();
		return Optional.ofNullable(authDataHolder).map(AuthorizationDataStore::getAuthorizationData).orElse(null);
	}

	@Override
	public void logout(HttpHeaders headers) {
		AuthorizationDataStore authDataHolder = AuthorizationDataHolder.getCredentialContext();
		authDataHolder.cleanUpAuthorizationStore();
	}
